...
This section contains a description of testing by class. The methods in Message Class and User Class have used JUnit tests. They are common to both Chatroom and Server folders. The methods in the remaining classes, primarily were part of either File IO or Network/Server interactions. They are tested through the main method, or being called by other functioning methods that have been called either in the main method or GUI's in UI package. Certain methods were used to test others and hence they were not required to be tested. Below is a description of each class and how its methods/constructors have been tested. 
...
**FrameUtil** Frame Util is GUI related and hence not tested. 

**UserInterfaceFrame** To test the delete message function, if two users send messages to each other (e.g. User1 sends message to User2), either User can click "delete" in the UserInterfaceFrame. The most recent message will be deleted simultaneously for both users, indicating that the feature is functioning properly. 
