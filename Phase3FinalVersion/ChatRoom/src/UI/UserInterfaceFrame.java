...
/**
 * Main user interface for the chat application, providing functionalities for messaging,
 * friend management, and user profile updates.
 *
 * <p>This class encapsulates all user interface components necessary for users to interact
 * with the chat application. It includes sending messages, managing friends, blocking users,
 * and updating profile pictures.</p>
 */
...
public class UserInterfaceFrame {
    ...
    private UserInterfaceFrame() {
        ...
        withdrawBtn.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                if (messages.size() == 0) {
                    JOptionPane.showMessageDialog(p1, "Recall failed, you haven't sent a message yet!");
                } else {
                    withdrawMsg();

                    JOptionPane.showMessageDialog(p1, "Recall successful！");
                }
    ...
    public void withdrawMsg() {
        strBuf.setLength(0);
        if (!messages.get(messages.size() - 1).getGetter().equals(currentUser.getUsername())) {
            userClientService.withdraw(messages.get(messages.size() - 1).getGetter(), "withdraw");
        } else {
            userClientService.withdrawMe(messages.get(messages.size() - 1).getSender(), "withdraw");
        }
        messages.remove(messages.size() - 1);
        for (int i = 0; i < messages.size(); i++) {
            strBuf.append(messages.get(i));
        }
        chatMsg.setText(strBuf.toString());
    }

    public void withdrawSendMsgMe(String sender, String getter) {
        strBuf.setLength(0);
        int index = -1;

        //messages.remove(messages.size() - 1);
        for (int i = 0; i < messages.size(); i++) {
            if (messages.get(i).getSender().equals(sender) && messages.get(i).getGetter().equals(getter)) {
                index = i;
            }
            // strBuf.append(messages.get(i));
        }
    ...
    public void withdrawSendMsg(String sender) {
        strBuf.setLength(0);
        int index = -1;
    ...
    public void init(User currentUser, ConcurrentHashMap<String, User> list,
                     UserClientService userClientService) {
        ...
        sendBtn.addActionListener(e -> {
            ...
            if (userNameSets.size() > 1) {
                JOptionPane.showMessageDialog(p1,
                        "Only one person can be sent a private message at a time!");
                ...
                for (String name : userNameSets) {
                    ...
                    if (users.get(name).isBlockedBy(currentUser.getUsername())) {
                        ...
                        msg.setGetter(name);
                        msg.setContent(textMsg.getText());
                        messages.add(msg);
                        strBuf.append(currentUser.getUsername() + " said to " + name + " :\n" + textMsg.getText() + "\n");
                        userClientService.Send(name,
                                textMsg.getText());
                        chatMsg.setText(strBuf.toString());
                        textMsg.setText("");
                    }
        ...
        sendAllBtn.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                userClientService.sendAll(currentUser.getUsername() +
                        " said to all:\n" + textAllMsg.getText() + "\n");
