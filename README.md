...
## Overview
The UserClientService class facilitates communication between the client and the server. It manages user authentication, retrieves online friend lists, sends messages, and ensures proper communication termination.

## Features
User authentication with the server.
Retrieval of online user lists.
Sending personal and broadcast messages.
Proper termination of communication upon user exit.

## Usage
To utilize this service, instantiate the UserClientService class and employ its methods for server communication based on user actions. Ensure the server is operational and accessible prior to invoking these methods.
...
## Usage
```java
Message msg = new Message("user1", "text");
msg.setContent("Hello, this is a test message.");
msg.setGetter("user2");
msg.setSendTime("2024-04-10 10:00");
msg.setMessageType("chat");

System.out.println("Message from " + msg.getSender() + " to " + msg.getGetter() +
                   ": " + msg.getContent() + " at " + msg.getSendTime());
```
...
## Components
- `JFrame` for the main window.
- `JTable` for displaying the user list.
- `JTextArea` for chat messages.
- `JButton` for various actions like sending messages and updating profile pictures.
- `JLabel` for displaying user information and statuses.
...
ConcurrentHashMap<String, User> users = FileUtil.getUsers();
if (users != null) {
    users.forEach((key, value) -> System.out.println(key + ": " + value.toString()));
}
```
...
myFrame.setSize(800, 600);  // Set the initial size of the frame
FrameUtil.center(myFrame);  // Center the frame on the screen
