...
/**
 * A frame used for user registration in a chat application.
 *
 * <p>This frame provides a user interface for new users to register by entering their personal information
 * and optionally uploading a profile picture. It checks for username uniqueness and saves the new user data.</p>
 */
    ...
    public RegisterFrame(ConcurrentHashMap<String, User> users) {
        ...
        registerButton.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                // Set user's Info
                setUsers();
                user.setUser(nameField1.getText(), usernameField1.getText(),
                        passwordField1.getText(), emilField1.getText());

                for (String key : users.keySet()) {
                    if (users.get(key).getUsername().equals(user.getUsername())) {
                        JOptionPane.showMessageDialog(root,
                                "The current username has already been registered,error!");
                        return;
                    }
                }
